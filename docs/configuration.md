...
| `USER_AGENT` | `src/fetch-constants.ts` | `Mozilla/5.0 AppleWebKit/537.36 (KHTML, like Gecko) Chrome/125.0.0.0 Safari/537.36` | User-Agent header for HTTP requests |
...
| `BINARY_TYPES` | `src/fetch-constants.ts` | Array of 8 MIME type prefixes | Content types rejected as binary (see below) |
| `BLOCKED_HOSTNAMES` | `src/ssrf.ts` | `["localhost", "127.0.0.1", "0.0.0.0", "[::1]"]` | Exact hostnames always blocked for SSRF |
| `BLOCKED_HOSTNAME_PREFIXES` | `src/ssrf.ts` | 19 prefixes (`10.`, `172.16.`–`172.31.`, `192.168.`, `169.254.`) | Private IP range prefixes blocked for SSRF |
...
DNS resolution is also performed to catch dynamic/private IPs not caught by static prefix matching. If DNS resolution fails entirely (both IPv4 and IPv6), the request is rejected with an error — this is a fail-closed design. See [docs/security.md](./security.md) for the full SSRF threat model.
...
- If truncated, the full content is written to a temp file at `os.tmpdir()/pi-fetch-{random}/content.md` where `{random}` is a unique directory name generated by `fs.mkdtemp()`.
...
```
{os.tmpdir()}/repository-{owner}/{repo}
```

The path is built with `path.join(tmpdir(), ...)` from Node.js `node:os`, making it platform-aware (e.g., `/tmp` on Unix, `%LOCALAPPDATA%\Temp` on Windows).

Example: `https://github.com/torvalds/linux` → `/tmp/repository-torvalds/linux` (on Linux)
...
- **NOT auto-cleaned**: Cloned repos persist at `os.tmpdir()/repository-{owner}/{repo}` indefinitely. This is intentional — the agent is expected to explore the cloned repository via subsequent tool calls (`read`, `grep`, `ls`, `bash`).
...
- **Windows reserved device names**: Owner and repo names matching reserved Windows device names (`CON`, `PRN`, `AUX`, `NUL`, `COM1`–`COM9`, `LPT1`–`LPT9`) are rejected on all platforms to prevent cross-platform filesystem issues.
...
On Windows, the global `pi` invocation uses `shell: true` to resolve `.cmd` wrappers. Node.js automatically shell-escapes array arguments (wraps them in double quotes), preventing `cmd.exe` from interpreting special characters in the task prompt.
...
- **Unix**: `SIGTERM` is sent to the subprocess. After `SIGKILL_DELAY_MS` (5 seconds), if the process is still alive, `SIGKILL` is sent.
- **Windows**: `proc.kill()` is called directly — signals don't work as on Unix, so there is no `SIGKILL` fallback.
...
The interfaces used across the extension are defined in `src/types.ts`:

- **`FetchContentDetails`** — Structured details returned alongside every tool result (URL, title, truncation status, repo owner/name/path, content type, etc.).
- **`SummarizeUpdate`** — Streaming update payload for summarization progress (carries content chunks and status).
