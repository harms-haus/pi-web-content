...
import { isBlockedHostname, validateUrlForSsrf, isBlockedByDns, validateRedirectForSsrf } from "../ssrf.js";
...
describe("validateUrlForSsrf", () => {
  ...
  describe("blocked hostnames throw", () => {
    it.each(["localhost", "127.0.0.1", "0.0.0.0", "[::1]"])(
      ...
      async (hostname) => {
        await expect(
          validateUrlForSsrf(`http://${hostname}/path`)
    ...
    it("throws for 10.x.x.x addresses", async () => {
      await expect(
        validateUrlForSsrf("http://10.0.0.1/path")
      ).rejects.toThrow(/Blocked/);
    });
  ...
  describe("DNS rebinding protection", () => {
    it("throws when DNS resolves to a private IP", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://dns-rebind.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
    ...
    it("throws when DNS resolves to a 127.x.x.x IP", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://loopback-dns.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
    ...
    it("throws when DNS resolves to a 192.168.x.x IP", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://private-dns.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
    ...
    it("throws when IPv6 DNS resolves to ::1", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://ipv6-loopback.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
    ...
    it("throws when IPv6 DNS resolves to fe80:: link-local", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://linklocal-ipv6.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
    ...
    it("throws when IPv6 DNS resolves to IPv4-mapped private address", async () => {
      ...
      await expect(
        validateUrlForSsrf("http://mapped-ipv6.example.com/")
      ).rejects.toThrow(/resolved IP.*internal/);
    });
  });
});
...
describe("validateRedirectForSsrf", () => {
  ...
  it("rejects redirect to ftp:// scheme", async () => {
    ...
    await expect(validateRedirectForSsrf(fromUrl, toUrl)).rejects.toThrow(/Blocked.*unsupported scheme/);
  });
  ...
  it("rejects redirect to IPv6 loopback [::1]", async () => {
    ...
    await expect(validateRedirectForSsrf(fromUrl, toUrl)).rejects.toThrow(/Blocked.*IPv6.*internal/);
  });
